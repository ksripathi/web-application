...
#+BEGIN_SRC python :tangle ../../src/obj.py :eval no
    def __init__(self,name,email):
        if  not WebSystem.system_active :
            ...
            self.users.append(adminstrator)
...
#+BEGIN_SRC python :tangle ../../src/obj.py :eval no
    def login(self,user):
        if isinstance(user,User) and user in WebSystem.users:
            flag = 0
            for sess in WebSystem.sessions:
    ...
    def update_role(self,session,user):
        if session.usr.role == Role.admin:
            if isinstance (user,User) and user in WebSystem.users:
                ...
                elif user.role == Role.user and user == session.usr and WebSystem.admin_check(user) == True:
...
#+BEGIN_SRC python :tangle ../../src/obj.py :eval no
    def update_name(self,session,user,name):
        if session.usr.role == Role.admin :
            ...
            if isinstance (user,User) and user in WebSystem.users and user == session.usr:
                user.set_name(name)
            else:
...
#+BEGIN_SRC python :tangle ../../src/obj.py :eval no
    def update_email(self,session,user,email):
            if session.usr.role == Role.admin :
                if isinstance (email,Email) and user in Websystem.users :
                    user.set_email(email)
                else:
                    raise TypeError('Invalid User type')
            
            elif session.usr.role == Role.user :
                if isinstance (email,Email) and user in Websystem.users and user == session.usr:
                    user.set_email(email)
                else:
                    raise TypeError('Invalid User type')
            else:
...
#+BEGIN_SRC python :tangle ../../src/obj.py :eval no
    def search_users(self,session,**kwargs):
        if session.usr in WebSystem.users:
            search = []
...
#+BEGIN_SRC python :tangle ../../src/obj.py :eval no
    def show_users(self):
        return WebSystem.users
#+END_SRC

**** get_sessions
#+BEGIN_SRC python :tangle ../../src/obj.py :eval no
    def get_sessions(self):
        return WebSystem.sessions
#+END_SRC

**** get_user_from_email
#+BEGIN_SRC python :tangle ../../src/obj.py :eval no
    def get_user_from_email(self,email):
        for users in WebSystem.users:
            if users.get_email().value == email:
                return users
        return False
#+END_SRC

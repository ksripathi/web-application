...
#+BEGIN_SRC python :tangle ../../src/db.py :eval no
    def to_client(self):
        return self.to_client(self.usr)
#+END_SRC
** WebSystem
    ...
    def login(self,user):
        if isinstance(user,User) and user in self.users:
            ...
            if flag == 0:
                ...
                au = Audit(session = json.dumps(sess.to_client()),entity = 'System',time = datetime.now(),operation = 'create session',status = 'Sucess')
                au.save()
                return sess 
            else:
                au = Audit(session = 'Session not created',entity = 'System',time = datetime.now(),operation = 'create session',status = 'Fail')
                au.save()
...
#+BEGIN_SRC python :tangle ../../src/db.py :eval no
    def logout(self,session):
        if isinstance(session,Session):
            ...
            au = Audit(session = json.dumps(session.to_client()),entity = 'System',time = datetime.now(),operation = 'delete session',status = 'Sucess')
            au.save()
        else:
            au = Audit(session = json.dumps(session.to_client()),entity = 'System',time = datetime.now(),operation = 'delete session',status = 'Fail')
            au.save()
...
#+BEGIN_SRC python :tangle ../../src/db.py :eval no
    def create_user(self,session,name,email,role):
	    ...
	    newUser.save()
	    self.users = User.get_all()
            au = Audit(session = json.dumps(session.to_client()),entity = 'User',time = datetime.now(),operation = 'create user',status = 'Sucess')
            au.save()
            return True
        else:
            au = Audit(session = json.dumps(session.to_client()),entity = 'User',time = datetime.now(),operation = 'create user',status = 'Fail')
            au.save()
...
#+BEGIN_SRC python :tangle ../../src/db.py :eval no
    def delete_user(self,session,user):
            ...
            if isinstance (user,User) and user in self.users:
                for sess in self.sessions:
                    if sess.usr.email.value == user.email.value:
                        au = Audit(session = json.dumps(session.to_client()),entity = 'User',time = datetime.now(),operation = 'delete user',status = 'Fail')
                        au.save()
		...
		if del_name == []:
		    del_name.delete()
		self.users = User.get_all()
                au = Audit(session = json.dumps(session.to_client()),entity = 'User',time = datetime.now(),operation = 'delete user',status = 'Sucess')
                au.save()
                return True
            else:
                au = Audit(session = json.dumps(session.to_client()),entity = 'User',time = datetime.now(),operation = 'delete user',status = 'Fail')
        ...
        if session.usr.role.value == 'admin':
            if isinstance (user,User) and user in self.users:
                if user.role.value == 'user':
                    ...
                    user.save()
                    self.users = User.get_all()
                    au = Audit(session = json.dumps(session.to_client()),entity = 'Role',time = datetime.now(),operation = 'update role',status = 'Sucess',old_data = 'user')
                    au.save()
                    return user
                    ...
                    if user.email.value == session.usr.email.value and self.admin_check(user) == True:
                        ...
                        user.save()
                        self.users = User.get_all()
                        au = Audit(session = json.dumps(session.to_client()),entity = 'Role',time = datetime.now(),operation = 'update role',status = 'Sucess',old_data = 'admin')
                        au.save()
                        return user
                    else:
                        au = Audit(session = json.dumps(session.to_client()),entity = 'Role',time = datetime.now(),operation = 'update role',status = 'Fail')
                        au.save()
        ...
        if session.usr.role.value == 'admin' :
            if isinstance (user,User) and user in self.users :
                ...
                if old_name.users == []:
                    old_name.delete()
                self.users = User.get_all()
                au = Audit(session = json.dumps(session.to_client()),entity = 'Name',time = datetime.now(),operation = 'update name',status = 'Sucess',old_data = old_name_str)
                au.save()
                return user
            else:
            ...
            if isinstance (user,User) and user in self.users and user.name.value == session.usr.name.value:
                ...
                au = Audit(session = json.dumps(session.to_client()),entity = 'Name',time = datetime.now(),operation = 'update name',status = 'Fail')
                au.save()
    ...
    def update_email(self,session,user,email):
            if session.usr.role.value == 'admin' :
                if isinstance (user,User) and user in self.users :
                    ...
                    old_email_str = old_email.value
                    user.set_email(em)
                    user.save()
                    old_email.delete()
                    self.users = User.get_all()
                    au = Audit(session = json.dumps(session.to_client()),entity = 'Email',time = datetime.now(),operation = 'update email',status = 'Sucess',old_data = old_email_str)
                    au.save()
                    return user
                else:
                    raise TypeError('Invalid User type')          
                ...
                if isinstance (user,User) and user in self.users and user.email.value == session.usr.email.value:
                    ...
                    au = Audit(session = json.dumps(session.to_client()),entity = 'Email',time = datetime.now(),operation = 'update email',status = 'Fail')
                    au.save()
                    raise NotAuthorizedError('Only an admin can change the names of other users')
            else:
                au = Audit(session = json.dumps(session.to_client()),entity = 'Email',time = datetime.now(),operation = 'update email',status = 'Fail')
...
** Audit
*** Introduction
   This class registers(logs) all the operations performed on the system.
*** Definition
#+BEGIN_SRC python :tangle ../../src/db.py :eval no
class Audit(Entity):
    id = db.Column(db.Integer,primary_key = True)
    ...
    old_data = db.Column(db.String(300))
...
#+BEGIN_SRC python :tangle ../../src/db.py :eval no
    ...
    def __init__(self,**kwargs):
        if 'session' not in kwargs:
            raise AttributeRequired('session in mandatory')
        if 'entity' not in kwargs:
            raise AttributeRequired('entity in mandatory')
        if 'time' not in kwargs:
            raise AttributeRequired('time in mandatory')
        if 'operation' not in kwargs:
            raise AttributeRequired('operation in mandatory')
        if 'status' not in kwargs:
            raise AttributeRequired('status in mandatory')
